...
##define some shapes to differentiate between tanks (this will be important due to the pseudoreplicate issue)
shps = c(19,15)
...
##results
##location with many DE (at least after the Wald test)
##time effect is a bit lilmited
summary(results(dds1, alpha=sig_lvl))  ##~Location + Year vs ~Location

'''
out of 20041 with nonzero total read count
...
##30 genes did not converge; for LRT you must slightly adapt the following command to remove these
...
clustering_data  %>% count(cluster, genes)
'''
cluster   genes  n
1         1 ATPase6 16
2         1  g13185 16
...
#because there were originally 16 clusters, each gene has 16 entries

clustering_data  %>% count(cluster) %>%

'''
cluster    n
1        1  576 ###these numbers must be divided by 16
2        4 1280
3        5  352
4        7  480
5        8  384
...
'''
labeller(cluster = c(1 = "Cluster 1: 36 genes",
             4 = "Cluster 2: 80 genes",
             5 = "Cluster 3: 22 genes",
             7 = "Cluster 4: 30",
             8 = "Cluster 5: 24",
             12 = "Cluster 6: 25",
             13 = "Cluster 7: 43",
...
##adding discrete trendline
svg('time_cluster_with_discrete_trendline_both_regions.svg')
...
###find genes in cluster 14(old)/8(new)
...
##seperate manually after marine region but do not cluster again

##North Sea
clustering_data_NS <- clustering_data[clustering_data$Marine_region == 'North_sea',]

pdf('time_cluster_without_smoother_NS.pdf', width = 5, height = 7)
ggplot(clustering_data_NS, aes(Year, value)) +
...
##Baltic Sea
clustering_data_BS <- clustering_data[clustering_data$Marine_region == 'Baltic_sea',]

pdf('time_cluster_without_smoother_BS.pdf', width = 5, height = 7)
ggplot(clustering_data_BS, aes(Year, value)) +
  geom_boxplot(color='#4B006E') +
  labs(y= "z-score of expression", x = "") +
  theme_bw()+
  theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1))+
  facet_wrap(vars(cluster), ncol = 2)
...
GO_ids = read.csv('C:/Users/mbras/Desktop/eelpout_heatstress_experiment/functional annotation/StringtieEggNog_Gene2GO.csv', sep=';', header = F)
...
##find the genes behind the BP terms GO:0010038 and GO:0046686

selected <- c("GO:0010038", "GO:0046686") 

sigGenes(GOdata)[sigGenes(GOdata)  %in% unlist(unname(genesInTerm(GOdata, selected[1])))] ##the order matters here

'''
[1] "g1082"  "g1479"  "g15051" "g15604" "g16284" "g1682"  "g16853" "g17859"
[9] "g18081" "g1941"  "g20777" "g21061" "g2180"  "g21974" "g24179" "g25127"
[17] "g2546"  "g26079" "g4169"  "g4963"  "g643"   "g660"   "g8550"  "g8664" 
[25] "g9863"
'''
#overlap g8550 & g1082


sigGenes(GOdata)[sigGenes(GOdata)  %in% unlist(unname(genesInTerm(GOdata, selected[2])))] ##the order matters here

'''
[1] "g16284" "g1682"  "g18081" "g1941"  "g21061" "g21974" "g26079" "g660"
'''
#no overlap
...
#DE <- res.MelvsDar
DE <- read.csv('C:/Users/mbras/Desktop/UPB - poolseq time line/DEseq/DEG_zoarces_VaMvsDar_apeglm.csv', row.names = 1)
...
###finally; check, how the heat responsive genes behave in the timeseries data
#load the data
...
##large clusters
svg('heatstress_cluster_in_timeseries_with_discrete_trendline_both_regions_large.svg')
...
##small clusters
svg('heatstress_cluster_in_timeseries_with_discrete_trendline_both_regions_small.svg')
...
ggdata <- read.csv ("/Users/mariebrasseur/Desktop/trier/UPB - poolseq timeline/DEseq/DESeq_timeseries_BP_enrichment_fisher_neu.csv", row.names = 1)


ggdata <- ggdata %>% 
  arrange(p.value.Fisher.elim) %>%  # arrange in descending order
  slice(1:10) ##top20   


ggdata$Term <- factor(ggdata$Term, levels = rev(ggdata$Term)) # fixes order
ggdata$p.value.Fisher.elim <- as.numeric(ggdata$p.value.Fisher.elim)


g1 <- ggplot(ggdata,
             aes(x = Term, y = Significant, size = Significant/Expected, fill = -log10(p.value.Fisher.elim))) +
  expand_limits(y = 7) +
  ...
  labs(
    title = 'Time responsive genes',
    subtitle = 'top 10 BP terms')+
...
pdf(file = "/Users/mariebrasseur/Desktop/time_responsive_genes_top10.pdf" , width = 10.5, height = 7)
g1
dev.off()
